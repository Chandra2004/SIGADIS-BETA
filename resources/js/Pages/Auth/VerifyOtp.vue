...
const handleOtpInput = (index, event) => {
    ...
    if (val && index < 5) {
        const nextInput = document.getElementById(`ibu-otp-${index + 1}`);
...
const handleOtpKeydown = (index, event) => {
    if (event.key === 'Backspace' && !otpDigits.value[index] && index > 0) {
        const prevInput = document.getElementById(`ibu-otp-${index - 1}`);
...
const resendOtp = () => {
    if (resendTimer.value === 0) {
        router.post(route('auth.pregnant.otp.send'), {
            phone_number: props.phoneNumber,
        }, {
            onSuccess: () => {
                startTimer();
...
<template>
    <Head title="Verifikasi Nomor WhatsApp — SIGADIS" />

    <AuthLayout
        panel-eyebrow="Verifikasi Ibu Hamil"
        panel-title="Masukkan Kode Verifikasi WhatsApp"
        panel-description="Kami telah mengirimkan 6 digit kode OTP ke nomor WhatsApp Anda untuk mengamankan akun dan verifikasi data kesehatan Anda."
    >
        <!-- Header -->
        <div class="text-center md:text-left mb-8">
            ...
            <p class="text-sm text-[#43474E] leading-relaxed">
                Kode OTP verifikasi telah dikirim via WhatsApp ke nomor <strong class="text-[#26292E]">{{ phoneNumber }}</strong>.
            </p>
        </div>
        ...
        <form class="space-y-6" @submit.prevent="submit">
            ...
            <div>
                ...
                <div class="flex justify-between gap-2 max-w-sm mx-auto">
                    <input
                        ...
                        maxlength="1"
                        :autofocus="i === 0"
                        class="w-12 h-14 text-center text-2xl font-extrabold border border-[#C3C6CF] rounded-2xl bg-[#FAF9FC] text-[#26292E] focus:bg-white focus:ring-2 focus:ring-[#F3AEC0]/50 focus:border-[#854E5E] focus:outline-none transition-all shadow-sm"
                ...
                <p v-if="form.errors.phone_number" class="text-xs text-[#BA1A1A] mt-2 font-medium text-center">
            ...
            <div class="text-center">
                <button
                    ...
                    class="text-xs font-semibold text-[#854E5E] hover:text-[#5E323F] disabled:text-[#8A8D96] transition-colors"
                    @click="resendOtp"
                >
                    <span v-if="resendTimer > 0">
                        Kirim Ulang Kode OTP dalam (00:{{ resendTimer.toString().padStart(2, '0') }})
                    </span>
                    <span v-else class="underline font-bold">
